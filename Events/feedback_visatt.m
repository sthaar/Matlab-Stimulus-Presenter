...
out = ['event.correct = imread(event.data); \r\n'...
    'event.incorrect = imread(event.data); \r\n'...
    'event.mis       = imread(event.data); \r\n'...
    ]; %may be multiline!
end
...
out =   [...
    ...
    'if '...
    '(strcmp(cue,''Small_Fixation.JPG'') && strcmp(visTarget,''gratingL.bmp'') && strcmp(key,''left''))||'...
    '(strcmp(cue,''Small_Fixation.JPG'') && strcmp(visTarget,''gratingC.bmp'') && strcmp(key,''down''))||'...
    '(strcmp(cue,''Small_Fixation.JPG'') && strcmp(visTarget,''gratingR.bmp'') && strcmp(key,''right''))||'...
    '(strcmp(cue,''Small_Fixation.JPG'') && strcmp(audTarget,''audiocheck.net_sin_500Hz_-3dBFS_.025s.wav'') && strcmp(key,''left''))||'...
    '(strcmp(cue,''Small_Fixation.JPG'') && strcmp(audTarget,''audiocheck.net_sin_1000Hz_-3dBFS_.025s.wav'') && strcmp(key,''down''))||'...
    '(strcmp(cue,''Small_Fixation.JPG'') && strcmp(audTarget,''audiocheck.net_sin_2000Hz_-3dBFS_.025s.wav'') && strcmp(key,''right'')) \r\n'...
    'feedback = ''Goed!!'';  \r\n reply.response = ''correct''; textCol = [0 255 0];                                          \r\n' ... 
    'elseif isnan(key);   \r\n  feedback = ''Gemist!''; textCol = [255 127 0]; reply.response = ''incorrect''; \r\n'...
    'else \r\n feedback = ''Fout!'';  \r\n reply.response = ''miss'';  textCol = [255 0 0];       \r\n end;                                      \r\n' ... 
        'Screen(''TextSize'',windowPtr, 40);                                \r\n' ... 
        'DrawFormattedText(windowPtr, feedback,''center'',''center'',textCol); \r\n' ...
...
questionStruct(9).name = 'Choose Dataset:';
questionStruct(9).sort = 'popupmenu';
questionStruct(9).data = datasets;

questionStruct(10).name = 'Choose Dataset:';
questionStruct(10).sort = 'popupmenu';
questionStruct(10).data = datasets;

questionStruct(11).name = 'Choose Dataset:';
questionStruct(11).sort = 'popupmenu';
questionStruct(11).data = datasets;
% for sort:
...
previousEvent.feedback.correct      = data(9).data;
previousEvent.feedback.incorrect    = data(10).data;
previousEvent.feedback.mis          = data(11).data;
